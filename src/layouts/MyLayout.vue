  ...
  <q-layout view="lHh Lpr lFf">
    <q-layout-header>
      <q-toolbar color="primary" :class="{nodrag: unlogged, drag: !unlogged}">
        ...
        <q-btn icon="ion-close-circle-outline" v-if="!unlogged && isAdmin" flat dense round @click="close"
    ...
    <q-page-container>
      <q-page class="flex flex-center over" :class="{'bg-black': unlogged}">
        ...
        <sesion v-if="sesion != null && !unlogged" :sesion="sesion" :socket="getSocket()" :username="username"
                :equipo="settings.equipo" ref="sesion"/>
        <p v-if="promo != null && !unlogged" class="info">
          Promocion valida
        </p>
      </q-page>
    </q-page-container>
  ...
  export default {
    ...
    methods: {
      ...
      login: function () {
        ...
        if (this.username === "close" && this.password === process.env.MASTER_PASS) {
          this.close();
          return;
        }
